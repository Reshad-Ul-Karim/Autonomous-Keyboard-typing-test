...
class KeyboardOverlay:
    def __init__(self, keyboard_config=None):
        ...
        # Standard key size is approximately 1/15th of keyboard width
        standard_key_px = self.KEYBOARD_WIDTH_PX / 19

        # Default configuration with pixel-based scaling
        self.config = {
            ...
            'shift_width': standard_key_px * 2.25,  # 2.33x standard key
...
def main():
    ...
    while True:
        ...
        # Create fresh overlay with current key states
        overlay = keyboard.create_base_overlay()

        # Calculate positioning with finer scale control
        target_width = int(keyboard.KEYBOARD_WIDTH_PX * fine_scale)
        target_height = int(keyboard.KEYBOARD_HEIGHT_PX * fine_scale)
        ...
        # Create mask for proper positioning
        ...
        for c in range(3):
            result[:, :, c] = result[:, :, c] * (1 - mask_alpha * opacity) + \
        ...
        if key == ord('q'):
            ...
            cv2.imwrite('keyboard_alignment.png', result)
            print("Saved alignment frame")
